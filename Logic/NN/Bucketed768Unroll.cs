...
namespace Lizard.Logic.NN
{
    public static unsafe partial class Bucketed768
    {
        ...
        public static int GetEvaluationUnrolled512AGE(Position pos)
        {
            ref Accumulator accumulator = ref *pos.State->Accumulator;
            var maxVec = VectorT.Create((short)QA);
            var zeroVec = VShort.Zero;
            ...
            var ourWeights   = (short*)(LayerWeights + (outputBucket * (HiddenSize * 2)));
            var theirWeights = (short*)(LayerWeights + (outputBucket * (HiddenSize * 2)) + HiddenSize);

            VShort* ws = stackalloc VShort[4];
            VShort* vs = stackalloc VShort[4];
            VShort* clipped = stackalloc VShort[4];
            VShort* muls = stackalloc VShort[4];
            VInt* squares = stackalloc VInt[4];

            #region R_STM

            ws[0] = VectorT.LoadAligned(ourWeights + 0 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 1 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 2 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 3 * N);
            vs[0] = VectorT.LoadAligned(ourData + 0 * N);
            vs[1] = VectorT.LoadAligned(ourData + 1 * N);
            vs[2] = VectorT.LoadAligned(ourData + 2 * N);
            vs[3] = VectorT.LoadAligned(ourData + 3 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            muls[0] = SIMDClass.MultiplyLow(clipped[0], ws[0]);
            muls[1] = SIMDClass.MultiplyLow(clipped[1], ws[1]);
            muls[2] = SIMDClass.MultiplyLow(clipped[2], ws[2]);
            muls[3] = SIMDClass.MultiplyLow(clipped[3], ws[3]);
            squares[0] = SIMDClass.MultiplyAddAdjacent(muls[0], clipped[0]);
            squares[1] = SIMDClass.MultiplyAddAdjacent(muls[1], clipped[1]);
            squares[2] = SIMDClass.MultiplyAddAdjacent(muls[2], clipped[2]);
            squares[3] = SIMDClass.MultiplyAddAdjacent(muls[3], clipped[3]);
            sumVec += squares[0];
            sumVec += squares[1];
            sumVec += squares[2];
            sumVec += squares[3];
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 4 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 5 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 6 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 7 * N);
            vs[0] = VectorT.LoadAligned(ourData + 4 * N);
            vs[1] = VectorT.LoadAligned(ourData + 5 * N);
            vs[2] = VectorT.LoadAligned(ourData + 6 * N);
            vs[3] = VectorT.LoadAligned(ourData + 7 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 8 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 9 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 10 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 11 * N);
            vs[0] = VectorT.LoadAligned(ourData + 8 * N);
            vs[1] = VectorT.LoadAligned(ourData + 9 * N);
            vs[2] = VectorT.LoadAligned(ourData + 10 * N);
            vs[3] = VectorT.LoadAligned(ourData + 11 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 12 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 13 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 14 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 15 * N);
            vs[0] = VectorT.LoadAligned(ourData + 12 * N);
            vs[1] = VectorT.LoadAligned(ourData + 13 * N);
            vs[2] = VectorT.LoadAligned(ourData + 14 * N);
            vs[3] = VectorT.LoadAligned(ourData + 15 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 16 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 17 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 18 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 19 * N);
            vs[0] = VectorT.LoadAligned(ourData + 16 * N);
            vs[1] = VectorT.LoadAligned(ourData + 17 * N);
            vs[2] = VectorT.LoadAligned(ourData + 18 * N);
            vs[3] = VectorT.LoadAligned(ourData + 19 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 20 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 21 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 22 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 23 * N);
            vs[0] = VectorT.LoadAligned(ourData + 20 * N);
            vs[1] = VectorT.LoadAligned(ourData + 21 * N);
            vs[2] = VectorT.LoadAligned(ourData + 22 * N);
            vs[3] = VectorT.LoadAligned(ourData + 23 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 24 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 25 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 26 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 27 * N);
            vs[0] = VectorT.LoadAligned(ourData + 24 * N);
            vs[1] = VectorT.LoadAligned(ourData + 25 * N);
            vs[2] = VectorT.LoadAligned(ourData + 26 * N);
            vs[3] = VectorT.LoadAligned(ourData + 27 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 28 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 29 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 30 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 31 * N);
            vs[0] = VectorT.LoadAligned(ourData + 28 * N);
            vs[1] = VectorT.LoadAligned(ourData + 29 * N);
            vs[2] = VectorT.LoadAligned(ourData + 30 * N);
            vs[3] = VectorT.LoadAligned(ourData + 31 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 32 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 33 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 34 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 35 * N);
            vs[0] = VectorT.LoadAligned(ourData + 32 * N);
            vs[1] = VectorT.LoadAligned(ourData + 33 * N);
            vs[2] = VectorT.LoadAligned(ourData + 34 * N);
            vs[3] = VectorT.LoadAligned(ourData + 35 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 36 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 37 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 38 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 39 * N);
            vs[0] = VectorT.LoadAligned(ourData + 36 * N);
            vs[1] = VectorT.LoadAligned(ourData + 37 * N);
            vs[2] = VectorT.LoadAligned(ourData + 38 * N);
            vs[3] = VectorT.LoadAligned(ourData + 39 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 40 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 41 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 42 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 43 * N);
            vs[0] = VectorT.LoadAligned(ourData + 40 * N);
            vs[1] = VectorT.LoadAligned(ourData + 41 * N);
            vs[2] = VectorT.LoadAligned(ourData + 42 * N);
            vs[3] = VectorT.LoadAligned(ourData + 43 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 44 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 45 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 46 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 47 * N);
            vs[0] = VectorT.LoadAligned(ourData + 44 * N);
            vs[1] = VectorT.LoadAligned(ourData + 45 * N);
            vs[2] = VectorT.LoadAligned(ourData + 46 * N);
            vs[3] = VectorT.LoadAligned(ourData + 47 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 48 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 49 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 50 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 51 * N);
            vs[0] = VectorT.LoadAligned(ourData + 48 * N);
            vs[1] = VectorT.LoadAligned(ourData + 49 * N);
            vs[2] = VectorT.LoadAligned(ourData + 50 * N);
            vs[3] = VectorT.LoadAligned(ourData + 51 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 52 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 53 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 54 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 55 * N);
            vs[0] = VectorT.LoadAligned(ourData + 52 * N);
            vs[1] = VectorT.LoadAligned(ourData + 53 * N);
            vs[2] = VectorT.LoadAligned(ourData + 54 * N);
            vs[3] = VectorT.LoadAligned(ourData + 55 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 56 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 57 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 58 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 59 * N);
            vs[0] = VectorT.LoadAligned(ourData + 56 * N);
            vs[1] = VectorT.LoadAligned(ourData + 57 * N);
            vs[2] = VectorT.LoadAligned(ourData + 58 * N);
            vs[3] = VectorT.LoadAligned(ourData + 59 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 60 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 61 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 62 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 63 * N);
            vs[0] = VectorT.LoadAligned(ourData + 60 * N);
            vs[1] = VectorT.LoadAligned(ourData + 61 * N);
            vs[2] = VectorT.LoadAligned(ourData + 62 * N);
            vs[3] = VectorT.LoadAligned(ourData + 63 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 64 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 65 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 66 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 67 * N);
            vs[0] = VectorT.LoadAligned(ourData + 64 * N);
            vs[1] = VectorT.LoadAligned(ourData + 65 * N);
            vs[2] = VectorT.LoadAligned(ourData + 66 * N);
            vs[3] = VectorT.LoadAligned(ourData + 67 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 68 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 69 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 70 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 71 * N);
            vs[0] = VectorT.LoadAligned(ourData + 68 * N);
            vs[1] = VectorT.LoadAligned(ourData + 69 * N);
            vs[2] = VectorT.LoadAligned(ourData + 70 * N);
            vs[3] = VectorT.LoadAligned(ourData + 71 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 72 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 73 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 74 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 75 * N);
            vs[0] = VectorT.LoadAligned(ourData + 72 * N);
            vs[1] = VectorT.LoadAligned(ourData + 73 * N);
            vs[2] = VectorT.LoadAligned(ourData + 74 * N);
            vs[3] = VectorT.LoadAligned(ourData + 75 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 76 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 77 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 78 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 79 * N);
            vs[0] = VectorT.LoadAligned(ourData + 76 * N);
            vs[1] = VectorT.LoadAligned(ourData + 77 * N);
            vs[2] = VectorT.LoadAligned(ourData + 78 * N);
            vs[3] = VectorT.LoadAligned(ourData + 79 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 80 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 81 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 82 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 83 * N);
            vs[0] = VectorT.LoadAligned(ourData + 80 * N);
            vs[1] = VectorT.LoadAligned(ourData + 81 * N);
            vs[2] = VectorT.LoadAligned(ourData + 82 * N);
            vs[3] = VectorT.LoadAligned(ourData + 83 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 84 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 85 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 86 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 87 * N);
            vs[0] = VectorT.LoadAligned(ourData + 84 * N);
            vs[1] = VectorT.LoadAligned(ourData + 85 * N);
            vs[2] = VectorT.LoadAligned(ourData + 86 * N);
            vs[3] = VectorT.LoadAligned(ourData + 87 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 88 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 89 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 90 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 91 * N);
            vs[0] = VectorT.LoadAligned(ourData + 88 * N);
            vs[1] = VectorT.LoadAligned(ourData + 89 * N);
            vs[2] = VectorT.LoadAligned(ourData + 90 * N);
            vs[3] = VectorT.LoadAligned(ourData + 91 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 92 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 93 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 94 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 95 * N);
            vs[0] = VectorT.LoadAligned(ourData + 92 * N);
            vs[1] = VectorT.LoadAligned(ourData + 93 * N);
            vs[2] = VectorT.LoadAligned(ourData + 94 * N);
            vs[3] = VectorT.LoadAligned(ourData + 95 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 96 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 97 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 98 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 99 * N);
            vs[0] = VectorT.LoadAligned(ourData + 96 * N);
            vs[1] = VectorT.LoadAligned(ourData + 97 * N);
            vs[2] = VectorT.LoadAligned(ourData + 98 * N);
            vs[3] = VectorT.LoadAligned(ourData + 99 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 100 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 101 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 102 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 103 * N);
            vs[0] = VectorT.LoadAligned(ourData + 100 * N);
            vs[1] = VectorT.LoadAligned(ourData + 101 * N);
            vs[2] = VectorT.LoadAligned(ourData + 102 * N);
            vs[3] = VectorT.LoadAligned(ourData + 103 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 104 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 105 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 106 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 107 * N);
            vs[0] = VectorT.LoadAligned(ourData + 104 * N);
            vs[1] = VectorT.LoadAligned(ourData + 105 * N);
            vs[2] = VectorT.LoadAligned(ourData + 106 * N);
            vs[3] = VectorT.LoadAligned(ourData + 107 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 108 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 109 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 110 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 111 * N);
            vs[0] = VectorT.LoadAligned(ourData + 108 * N);
            vs[1] = VectorT.LoadAligned(ourData + 109 * N);
            vs[2] = VectorT.LoadAligned(ourData + 110 * N);
            vs[3] = VectorT.LoadAligned(ourData + 111 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 112 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 113 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 114 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 115 * N);
            vs[0] = VectorT.LoadAligned(ourData + 112 * N);
            vs[1] = VectorT.LoadAligned(ourData + 113 * N);
            vs[2] = VectorT.LoadAligned(ourData + 114 * N);
            vs[3] = VectorT.LoadAligned(ourData + 115 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 116 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 117 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 118 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 119 * N);
            vs[0] = VectorT.LoadAligned(ourData + 116 * N);
            vs[1] = VectorT.LoadAligned(ourData + 117 * N);
            vs[2] = VectorT.LoadAligned(ourData + 118 * N);
            vs[3] = VectorT.LoadAligned(ourData + 119 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 120 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 121 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 122 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 123 * N);
            vs[0] = VectorT.LoadAligned(ourData + 120 * N);
            vs[1] = VectorT.LoadAligned(ourData + 121 * N);
            vs[2] = VectorT.LoadAligned(ourData + 122 * N);
            vs[3] = VectorT.LoadAligned(ourData + 123 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(ourWeights + 124 * N);
            ws[1] = VectorT.LoadAligned(ourWeights + 125 * N);
            ws[2] = VectorT.LoadAligned(ourWeights + 126 * N);
            ws[3] = VectorT.LoadAligned(ourWeights + 127 * N);
            vs[0] = VectorT.LoadAligned(ourData + 124 * N);
            vs[1] = VectorT.LoadAligned(ourData + 125 * N);
            vs[2] = VectorT.LoadAligned(ourData + 126 * N);
            vs[3] = VectorT.LoadAligned(ourData + 127 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 0 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 1 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 2 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 3 * N);
            vs[0] = VectorT.LoadAligned(theirData + 0 * N);
            vs[1] = VectorT.LoadAligned(theirData + 1 * N);
            vs[2] = VectorT.LoadAligned(theirData + 2 * N);
            vs[3] = VectorT.LoadAligned(theirData + 3 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 4 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 5 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 6 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 7 * N);
            vs[0] = VectorT.LoadAligned(theirData + 4 * N);
            vs[1] = VectorT.LoadAligned(theirData + 5 * N);
            vs[2] = VectorT.LoadAligned(theirData + 6 * N);
            vs[3] = VectorT.LoadAligned(theirData + 7 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 8 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 9 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 10 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 11 * N);
            vs[0] = VectorT.LoadAligned(theirData + 8 * N);
            vs[1] = VectorT.LoadAligned(theirData + 9 * N);
            vs[2] = VectorT.LoadAligned(theirData + 10 * N);
            vs[3] = VectorT.LoadAligned(theirData + 11 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 12 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 13 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 14 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 15 * N);
            vs[0] = VectorT.LoadAligned(theirData + 12 * N);
            vs[1] = VectorT.LoadAligned(theirData + 13 * N);
            vs[2] = VectorT.LoadAligned(theirData + 14 * N);
            vs[3] = VectorT.LoadAligned(theirData + 15 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 16 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 17 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 18 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 19 * N);
            vs[0] = VectorT.LoadAligned(theirData + 16 * N);
            vs[1] = VectorT.LoadAligned(theirData + 17 * N);
            vs[2] = VectorT.LoadAligned(theirData + 18 * N);
            vs[3] = VectorT.LoadAligned(theirData + 19 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 20 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 21 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 22 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 23 * N);
            vs[0] = VectorT.LoadAligned(theirData + 20 * N);
            vs[1] = VectorT.LoadAligned(theirData + 21 * N);
            vs[2] = VectorT.LoadAligned(theirData + 22 * N);
            vs[3] = VectorT.LoadAligned(theirData + 23 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 24 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 25 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 26 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 27 * N);
            vs[0] = VectorT.LoadAligned(theirData + 24 * N);
            vs[1] = VectorT.LoadAligned(theirData + 25 * N);
            vs[2] = VectorT.LoadAligned(theirData + 26 * N);
            vs[3] = VectorT.LoadAligned(theirData + 27 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 28 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 29 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 30 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 31 * N);
            vs[0] = VectorT.LoadAligned(theirData + 28 * N);
            vs[1] = VectorT.LoadAligned(theirData + 29 * N);
            vs[2] = VectorT.LoadAligned(theirData + 30 * N);
            vs[3] = VectorT.LoadAligned(theirData + 31 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 32 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 33 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 34 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 35 * N);
            vs[0] = VectorT.LoadAligned(theirData + 32 * N);
            vs[1] = VectorT.LoadAligned(theirData + 33 * N);
            vs[2] = VectorT.LoadAligned(theirData + 34 * N);
            vs[3] = VectorT.LoadAligned(theirData + 35 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 36 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 37 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 38 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 39 * N);
            vs[0] = VectorT.LoadAligned(theirData + 36 * N);
            vs[1] = VectorT.LoadAligned(theirData + 37 * N);
            vs[2] = VectorT.LoadAligned(theirData + 38 * N);
            vs[3] = VectorT.LoadAligned(theirData + 39 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 40 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 41 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 42 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 43 * N);
            vs[0] = VectorT.LoadAligned(theirData + 40 * N);
            vs[1] = VectorT.LoadAligned(theirData + 41 * N);
            vs[2] = VectorT.LoadAligned(theirData + 42 * N);
            vs[3] = VectorT.LoadAligned(theirData + 43 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 44 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 45 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 46 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 47 * N);
            vs[0] = VectorT.LoadAligned(theirData + 44 * N);
            vs[1] = VectorT.LoadAligned(theirData + 45 * N);
            vs[2] = VectorT.LoadAligned(theirData + 46 * N);
            vs[3] = VectorT.LoadAligned(theirData + 47 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 48 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 49 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 50 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 51 * N);
            vs[0] = VectorT.LoadAligned(theirData + 48 * N);
            vs[1] = VectorT.LoadAligned(theirData + 49 * N);
            vs[2] = VectorT.LoadAligned(theirData + 50 * N);
            vs[3] = VectorT.LoadAligned(theirData + 51 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 52 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 53 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 54 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 55 * N);
            vs[0] = VectorT.LoadAligned(theirData + 52 * N);
            vs[1] = VectorT.LoadAligned(theirData + 53 * N);
            vs[2] = VectorT.LoadAligned(theirData + 54 * N);
            vs[3] = VectorT.LoadAligned(theirData + 55 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 56 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 57 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 58 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 59 * N);
            vs[0] = VectorT.LoadAligned(theirData + 56 * N);
            vs[1] = VectorT.LoadAligned(theirData + 57 * N);
            vs[2] = VectorT.LoadAligned(theirData + 58 * N);
            vs[3] = VectorT.LoadAligned(theirData + 59 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 60 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 61 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 62 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 63 * N);
            vs[0] = VectorT.LoadAligned(theirData + 60 * N);
            vs[1] = VectorT.LoadAligned(theirData + 61 * N);
            vs[2] = VectorT.LoadAligned(theirData + 62 * N);
            vs[3] = VectorT.LoadAligned(theirData + 63 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 64 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 65 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 66 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 67 * N);
            vs[0] = VectorT.LoadAligned(theirData + 64 * N);
            vs[1] = VectorT.LoadAligned(theirData + 65 * N);
            vs[2] = VectorT.LoadAligned(theirData + 66 * N);
            vs[3] = VectorT.LoadAligned(theirData + 67 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 68 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 69 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 70 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 71 * N);
            vs[0] = VectorT.LoadAligned(theirData + 68 * N);
            vs[1] = VectorT.LoadAligned(theirData + 69 * N);
            vs[2] = VectorT.LoadAligned(theirData + 70 * N);
            vs[3] = VectorT.LoadAligned(theirData + 71 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 72 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 73 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 74 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 75 * N);
            vs[0] = VectorT.LoadAligned(theirData + 72 * N);
            vs[1] = VectorT.LoadAligned(theirData + 73 * N);
            vs[2] = VectorT.LoadAligned(theirData + 74 * N);
            vs[3] = VectorT.LoadAligned(theirData + 75 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 76 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 77 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 78 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 79 * N);
            vs[0] = VectorT.LoadAligned(theirData + 76 * N);
            vs[1] = VectorT.LoadAligned(theirData + 77 * N);
            vs[2] = VectorT.LoadAligned(theirData + 78 * N);
            vs[3] = VectorT.LoadAligned(theirData + 79 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 80 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 81 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 82 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 83 * N);
            vs[0] = VectorT.LoadAligned(theirData + 80 * N);
            vs[1] = VectorT.LoadAligned(theirData + 81 * N);
            vs[2] = VectorT.LoadAligned(theirData + 82 * N);
            vs[3] = VectorT.LoadAligned(theirData + 83 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 84 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 85 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 86 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 87 * N);
            vs[0] = VectorT.LoadAligned(theirData + 84 * N);
            vs[1] = VectorT.LoadAligned(theirData + 85 * N);
            vs[2] = VectorT.LoadAligned(theirData + 86 * N);
            vs[3] = VectorT.LoadAligned(theirData + 87 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 88 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 89 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 90 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 91 * N);
            vs[0] = VectorT.LoadAligned(theirData + 88 * N);
            vs[1] = VectorT.LoadAligned(theirData + 89 * N);
            vs[2] = VectorT.LoadAligned(theirData + 90 * N);
            vs[3] = VectorT.LoadAligned(theirData + 91 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 92 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 93 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 94 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 95 * N);
            vs[0] = VectorT.LoadAligned(theirData + 92 * N);
            vs[1] = VectorT.LoadAligned(theirData + 93 * N);
            vs[2] = VectorT.LoadAligned(theirData + 94 * N);
            vs[3] = VectorT.LoadAligned(theirData + 95 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 96 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 97 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 98 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 99 * N);
            vs[0] = VectorT.LoadAligned(theirData + 96 * N);
            vs[1] = VectorT.LoadAligned(theirData + 97 * N);
            vs[2] = VectorT.LoadAligned(theirData + 98 * N);
            vs[3] = VectorT.LoadAligned(theirData + 99 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 100 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 101 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 102 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 103 * N);
            vs[0] = VectorT.LoadAligned(theirData + 100 * N);
            vs[1] = VectorT.LoadAligned(theirData + 101 * N);
            vs[2] = VectorT.LoadAligned(theirData + 102 * N);
            vs[3] = VectorT.LoadAligned(theirData + 103 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 104 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 105 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 106 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 107 * N);
            vs[0] = VectorT.LoadAligned(theirData + 104 * N);
            vs[1] = VectorT.LoadAligned(theirData + 105 * N);
            vs[2] = VectorT.LoadAligned(theirData + 106 * N);
            vs[3] = VectorT.LoadAligned(theirData + 107 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 108 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 109 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 110 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 111 * N);
            vs[0] = VectorT.LoadAligned(theirData + 108 * N);
            vs[1] = VectorT.LoadAligned(theirData + 109 * N);
            vs[2] = VectorT.LoadAligned(theirData + 110 * N);
            vs[3] = VectorT.LoadAligned(theirData + 111 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 112 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 113 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 114 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 115 * N);
            vs[0] = VectorT.LoadAligned(theirData + 112 * N);
            vs[1] = VectorT.LoadAligned(theirData + 113 * N);
            vs[2] = VectorT.LoadAligned(theirData + 114 * N);
            vs[3] = VectorT.LoadAligned(theirData + 115 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 116 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 117 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 118 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 119 * N);
            vs[0] = VectorT.LoadAligned(theirData + 116 * N);
            vs[1] = VectorT.LoadAligned(theirData + 117 * N);
            vs[2] = VectorT.LoadAligned(theirData + 118 * N);
            vs[3] = VectorT.LoadAligned(theirData + 119 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 120 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 121 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 122 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 123 * N);
            vs[0] = VectorT.LoadAligned(theirData + 120 * N);
            vs[1] = VectorT.LoadAligned(theirData + 121 * N);
            vs[2] = VectorT.LoadAligned(theirData + 122 * N);
            vs[3] = VectorT.LoadAligned(theirData + 123 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
            ...
            ws[0] = VectorT.LoadAligned(theirWeights + 124 * N);
            ws[1] = VectorT.LoadAligned(theirWeights + 125 * N);
            ws[2] = VectorT.LoadAligned(theirWeights + 126 * N);
            ws[3] = VectorT.LoadAligned(theirWeights + 127 * N);
            vs[0] = VectorT.LoadAligned(theirData + 124 * N);
            vs[1] = VectorT.LoadAligned(theirData + 125 * N);
            vs[2] = VectorT.LoadAligned(theirData + 126 * N);
            vs[3] = VectorT.LoadAligned(theirData + 127 * N);
            clipped[0] = VectorT.Min(VectorT.Max(vs[0], zeroVec), maxVec);
            clipped[1] = VectorT.Min(VectorT.Max(vs[1], zeroVec), maxVec);
            clipped[2] = VectorT.Min(VectorT.Max(vs[2], zeroVec), maxVec);
            clipped[3] = VectorT.Min(VectorT.Max(vs[3], zeroVec), maxVec);
        ...
        public static int GetEvaluationUnrolled512AGE2(Position pos)
        {
            ref Accumulator accumulator = ref *pos.State->Accumulator;
            var maxVec = VectorT.Create((short)QA);
            var zeroVec = VShort.Zero;
            ...
            VShort w0, w1, w2, w3;
            VShort v0, v1, v2, v3;
            VShort clip0, clip1, clip2, clip3;
            VShort muls0, muls1, muls2, muls3;
            VInt squares0, squares1, squares2, squares3;

            #region R_STM

            w0 = VectorT.LoadAligned(ourWeights + 0 * N);
            w1 = VectorT.LoadAligned(ourWeights + 1 * N);
            w2 = VectorT.LoadAligned(ourWeights + 2 * N);
            w3 = VectorT.LoadAligned(ourWeights + 3 * N);
            v0 = VectorT.LoadAligned(ourData + 0 * N);
            v1 = VectorT.LoadAligned(ourData + 1 * N);
            v2 = VectorT.LoadAligned(ourData + 2 * N);
            v3 = VectorT.LoadAligned(ourData + 3 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            muls0 = SIMDClass.MultiplyLow(clip0, w0);
            muls1 = SIMDClass.MultiplyLow(clip1, w1);
            muls2 = SIMDClass.MultiplyLow(clip2, w2);
            muls3 = SIMDClass.MultiplyLow(clip3, w3);
            squares0 = SIMDClass.MultiplyAddAdjacent(muls0, clip0);
            squares1 = SIMDClass.MultiplyAddAdjacent(muls1, clip1);
            squares2 = SIMDClass.MultiplyAddAdjacent(muls2, clip2);
            squares3 = SIMDClass.MultiplyAddAdjacent(muls3, clip3);
            sumVec += squares0;
            sumVec += squares1;
            sumVec += squares2;
            sumVec += squares3;

            w0 = VectorT.LoadAligned(ourWeights + 4 * N);
            w1 = VectorT.LoadAligned(ourWeights + 5 * N);
            w2 = VectorT.LoadAligned(ourWeights + 6 * N);
            w3 = VectorT.LoadAligned(ourWeights + 7 * N);
            v0 = VectorT.LoadAligned(ourData + 4 * N);
            v1 = VectorT.LoadAligned(ourData + 5 * N);
            v2 = VectorT.LoadAligned(ourData + 6 * N);
            v3 = VectorT.LoadAligned(ourData + 7 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 8 * N);
            w1 = VectorT.LoadAligned(ourWeights + 9 * N);
            w2 = VectorT.LoadAligned(ourWeights + 10 * N);
            w3 = VectorT.LoadAligned(ourWeights + 11 * N);
            v0 = VectorT.LoadAligned(ourData + 8 * N);
            v1 = VectorT.LoadAligned(ourData + 9 * N);
            v2 = VectorT.LoadAligned(ourData + 10 * N);
            v3 = VectorT.LoadAligned(ourData + 11 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 12 * N);
            w1 = VectorT.LoadAligned(ourWeights + 13 * N);
            w2 = VectorT.LoadAligned(ourWeights + 14 * N);
            w3 = VectorT.LoadAligned(ourWeights + 15 * N);
            v0 = VectorT.LoadAligned(ourData + 12 * N);
            v1 = VectorT.LoadAligned(ourData + 13 * N);
            v2 = VectorT.LoadAligned(ourData + 14 * N);
            v3 = VectorT.LoadAligned(ourData + 15 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 16 * N);
            w1 = VectorT.LoadAligned(ourWeights + 17 * N);
            w2 = VectorT.LoadAligned(ourWeights + 18 * N);
            w3 = VectorT.LoadAligned(ourWeights + 19 * N);
            v0 = VectorT.LoadAligned(ourData + 16 * N);
            v1 = VectorT.LoadAligned(ourData + 17 * N);
            v2 = VectorT.LoadAligned(ourData + 18 * N);
            v3 = VectorT.LoadAligned(ourData + 19 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 20 * N);
            w1 = VectorT.LoadAligned(ourWeights + 21 * N);
            w2 = VectorT.LoadAligned(ourWeights + 22 * N);
            w3 = VectorT.LoadAligned(ourWeights + 23 * N);
            v0 = VectorT.LoadAligned(ourData + 20 * N);
            v1 = VectorT.LoadAligned(ourData + 21 * N);
            v2 = VectorT.LoadAligned(ourData + 22 * N);
            v3 = VectorT.LoadAligned(ourData + 23 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 24 * N);
            w1 = VectorT.LoadAligned(ourWeights + 25 * N);
            w2 = VectorT.LoadAligned(ourWeights + 26 * N);
            w3 = VectorT.LoadAligned(ourWeights + 27 * N);
            v0 = VectorT.LoadAligned(ourData + 24 * N);
            v1 = VectorT.LoadAligned(ourData + 25 * N);
            v2 = VectorT.LoadAligned(ourData + 26 * N);
            v3 = VectorT.LoadAligned(ourData + 27 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 28 * N);
            w1 = VectorT.LoadAligned(ourWeights + 29 * N);
            w2 = VectorT.LoadAligned(ourWeights + 30 * N);
            w3 = VectorT.LoadAligned(ourWeights + 31 * N);
            v0 = VectorT.LoadAligned(ourData + 28 * N);
            v1 = VectorT.LoadAligned(ourData + 29 * N);
            v2 = VectorT.LoadAligned(ourData + 30 * N);
            v3 = VectorT.LoadAligned(ourData + 31 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 32 * N);
            w1 = VectorT.LoadAligned(ourWeights + 33 * N);
            w2 = VectorT.LoadAligned(ourWeights + 34 * N);
            w3 = VectorT.LoadAligned(ourWeights + 35 * N);
            v0 = VectorT.LoadAligned(ourData + 32 * N);
            v1 = VectorT.LoadAligned(ourData + 33 * N);
            v2 = VectorT.LoadAligned(ourData + 34 * N);
            v3 = VectorT.LoadAligned(ourData + 35 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 36 * N);
            w1 = VectorT.LoadAligned(ourWeights + 37 * N);
            w2 = VectorT.LoadAligned(ourWeights + 38 * N);
            w3 = VectorT.LoadAligned(ourWeights + 39 * N);
            v0 = VectorT.LoadAligned(ourData + 36 * N);
            v1 = VectorT.LoadAligned(ourData + 37 * N);
            v2 = VectorT.LoadAligned(ourData + 38 * N);
            v3 = VectorT.LoadAligned(ourData + 39 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 40 * N);
            w1 = VectorT.LoadAligned(ourWeights + 41 * N);
            w2 = VectorT.LoadAligned(ourWeights + 42 * N);
            w3 = VectorT.LoadAligned(ourWeights + 43 * N);
            v0 = VectorT.LoadAligned(ourData + 40 * N);
            v1 = VectorT.LoadAligned(ourData + 41 * N);
            v2 = VectorT.LoadAligned(ourData + 42 * N);
            v3 = VectorT.LoadAligned(ourData + 43 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 44 * N);
            w1 = VectorT.LoadAligned(ourWeights + 45 * N);
            w2 = VectorT.LoadAligned(ourWeights + 46 * N);
            w3 = VectorT.LoadAligned(ourWeights + 47 * N);
            v0 = VectorT.LoadAligned(ourData + 44 * N);
            v1 = VectorT.LoadAligned(ourData + 45 * N);
            v2 = VectorT.LoadAligned(ourData + 46 * N);
            v3 = VectorT.LoadAligned(ourData + 47 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 48 * N);
            w1 = VectorT.LoadAligned(ourWeights + 49 * N);
            w2 = VectorT.LoadAligned(ourWeights + 50 * N);
            w3 = VectorT.LoadAligned(ourWeights + 51 * N);
            v0 = VectorT.LoadAligned(ourData + 48 * N);
            v1 = VectorT.LoadAligned(ourData + 49 * N);
            v2 = VectorT.LoadAligned(ourData + 50 * N);
            v3 = VectorT.LoadAligned(ourData + 51 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 52 * N);
            w1 = VectorT.LoadAligned(ourWeights + 53 * N);
            w2 = VectorT.LoadAligned(ourWeights + 54 * N);
            w3 = VectorT.LoadAligned(ourWeights + 55 * N);
            v0 = VectorT.LoadAligned(ourData + 52 * N);
            v1 = VectorT.LoadAligned(ourData + 53 * N);
            v2 = VectorT.LoadAligned(ourData + 54 * N);
            v3 = VectorT.LoadAligned(ourData + 55 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 56 * N);
            w1 = VectorT.LoadAligned(ourWeights + 57 * N);
            w2 = VectorT.LoadAligned(ourWeights + 58 * N);
            w3 = VectorT.LoadAligned(ourWeights + 59 * N);
            v0 = VectorT.LoadAligned(ourData + 56 * N);
            v1 = VectorT.LoadAligned(ourData + 57 * N);
            v2 = VectorT.LoadAligned(ourData + 58 * N);
            v3 = VectorT.LoadAligned(ourData + 59 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 60 * N);
            w1 = VectorT.LoadAligned(ourWeights + 61 * N);
            w2 = VectorT.LoadAligned(ourWeights + 62 * N);
            w3 = VectorT.LoadAligned(ourWeights + 63 * N);
            v0 = VectorT.LoadAligned(ourData + 60 * N);
            v1 = VectorT.LoadAligned(ourData + 61 * N);
            v2 = VectorT.LoadAligned(ourData + 62 * N);
            v3 = VectorT.LoadAligned(ourData + 63 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 64 * N);
            w1 = VectorT.LoadAligned(ourWeights + 65 * N);
            w2 = VectorT.LoadAligned(ourWeights + 66 * N);
            w3 = VectorT.LoadAligned(ourWeights + 67 * N);
            v0 = VectorT.LoadAligned(ourData + 64 * N);
            v1 = VectorT.LoadAligned(ourData + 65 * N);
            v2 = VectorT.LoadAligned(ourData + 66 * N);
            v3 = VectorT.LoadAligned(ourData + 67 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 68 * N);
            w1 = VectorT.LoadAligned(ourWeights + 69 * N);
            w2 = VectorT.LoadAligned(ourWeights + 70 * N);
            w3 = VectorT.LoadAligned(ourWeights + 71 * N);
            v0 = VectorT.LoadAligned(ourData + 68 * N);
            v1 = VectorT.LoadAligned(ourData + 69 * N);
            v2 = VectorT.LoadAligned(ourData + 70 * N);
            v3 = VectorT.LoadAligned(ourData + 71 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 72 * N);
            w1 = VectorT.LoadAligned(ourWeights + 73 * N);
            w2 = VectorT.LoadAligned(ourWeights + 74 * N);
            w3 = VectorT.LoadAligned(ourWeights + 75 * N);
            v0 = VectorT.LoadAligned(ourData + 72 * N);
            v1 = VectorT.LoadAligned(ourData + 73 * N);
            v2 = VectorT.LoadAligned(ourData + 74 * N);
            v3 = VectorT.LoadAligned(ourData + 75 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 76 * N);
            w1 = VectorT.LoadAligned(ourWeights + 77 * N);
            w2 = VectorT.LoadAligned(ourWeights + 78 * N);
            w3 = VectorT.LoadAligned(ourWeights + 79 * N);
            v0 = VectorT.LoadAligned(ourData + 76 * N);
            v1 = VectorT.LoadAligned(ourData + 77 * N);
            v2 = VectorT.LoadAligned(ourData + 78 * N);
            v3 = VectorT.LoadAligned(ourData + 79 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 80 * N);
            w1 = VectorT.LoadAligned(ourWeights + 81 * N);
            w2 = VectorT.LoadAligned(ourWeights + 82 * N);
            w3 = VectorT.LoadAligned(ourWeights + 83 * N);
            v0 = VectorT.LoadAligned(ourData + 80 * N);
            v1 = VectorT.LoadAligned(ourData + 81 * N);
            v2 = VectorT.LoadAligned(ourData + 82 * N);
            v3 = VectorT.LoadAligned(ourData + 83 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 84 * N);
            w1 = VectorT.LoadAligned(ourWeights + 85 * N);
            w2 = VectorT.LoadAligned(ourWeights + 86 * N);
            w3 = VectorT.LoadAligned(ourWeights + 87 * N);
            v0 = VectorT.LoadAligned(ourData + 84 * N);
            v1 = VectorT.LoadAligned(ourData + 85 * N);
            v2 = VectorT.LoadAligned(ourData + 86 * N);
            v3 = VectorT.LoadAligned(ourData + 87 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 88 * N);
            w1 = VectorT.LoadAligned(ourWeights + 89 * N);
            w2 = VectorT.LoadAligned(ourWeights + 90 * N);
            w3 = VectorT.LoadAligned(ourWeights + 91 * N);
            v0 = VectorT.LoadAligned(ourData + 88 * N);
            v1 = VectorT.LoadAligned(ourData + 89 * N);
            v2 = VectorT.LoadAligned(ourData + 90 * N);
            v3 = VectorT.LoadAligned(ourData + 91 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 92 * N);
            w1 = VectorT.LoadAligned(ourWeights + 93 * N);
            w2 = VectorT.LoadAligned(ourWeights + 94 * N);
            w3 = VectorT.LoadAligned(ourWeights + 95 * N);
            v0 = VectorT.LoadAligned(ourData + 92 * N);
            v1 = VectorT.LoadAligned(ourData + 93 * N);
            v2 = VectorT.LoadAligned(ourData + 94 * N);
            v3 = VectorT.LoadAligned(ourData + 95 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 96 * N);
            w1 = VectorT.LoadAligned(ourWeights + 97 * N);
            w2 = VectorT.LoadAligned(ourWeights + 98 * N);
            w3 = VectorT.LoadAligned(ourWeights + 99 * N);
            v0 = VectorT.LoadAligned(ourData + 96 * N);
            v1 = VectorT.LoadAligned(ourData + 97 * N);
            v2 = VectorT.LoadAligned(ourData + 98 * N);
            v3 = VectorT.LoadAligned(ourData + 99 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 100 * N);
            w1 = VectorT.LoadAligned(ourWeights + 101 * N);
            w2 = VectorT.LoadAligned(ourWeights + 102 * N);
            w3 = VectorT.LoadAligned(ourWeights + 103 * N);
            v0 = VectorT.LoadAligned(ourData + 100 * N);
            v1 = VectorT.LoadAligned(ourData + 101 * N);
            v2 = VectorT.LoadAligned(ourData + 102 * N);
            v3 = VectorT.LoadAligned(ourData + 103 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 104 * N);
            w1 = VectorT.LoadAligned(ourWeights + 105 * N);
            w2 = VectorT.LoadAligned(ourWeights + 106 * N);
            w3 = VectorT.LoadAligned(ourWeights + 107 * N);
            v0 = VectorT.LoadAligned(ourData + 104 * N);
            v1 = VectorT.LoadAligned(ourData + 105 * N);
            v2 = VectorT.LoadAligned(ourData + 106 * N);
            v3 = VectorT.LoadAligned(ourData + 107 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 108 * N);
            w1 = VectorT.LoadAligned(ourWeights + 109 * N);
            w2 = VectorT.LoadAligned(ourWeights + 110 * N);
            w3 = VectorT.LoadAligned(ourWeights + 111 * N);
            v0 = VectorT.LoadAligned(ourData + 108 * N);
            v1 = VectorT.LoadAligned(ourData + 109 * N);
            v2 = VectorT.LoadAligned(ourData + 110 * N);
            v3 = VectorT.LoadAligned(ourData + 111 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 112 * N);
            w1 = VectorT.LoadAligned(ourWeights + 113 * N);
            w2 = VectorT.LoadAligned(ourWeights + 114 * N);
            w3 = VectorT.LoadAligned(ourWeights + 115 * N);
            v0 = VectorT.LoadAligned(ourData + 112 * N);
            v1 = VectorT.LoadAligned(ourData + 113 * N);
            v2 = VectorT.LoadAligned(ourData + 114 * N);
            v3 = VectorT.LoadAligned(ourData + 115 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 116 * N);
            w1 = VectorT.LoadAligned(ourWeights + 117 * N);
            w2 = VectorT.LoadAligned(ourWeights + 118 * N);
            w3 = VectorT.LoadAligned(ourWeights + 119 * N);
            v0 = VectorT.LoadAligned(ourData + 116 * N);
            v1 = VectorT.LoadAligned(ourData + 117 * N);
            v2 = VectorT.LoadAligned(ourData + 118 * N);
            v3 = VectorT.LoadAligned(ourData + 119 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 120 * N);
            w1 = VectorT.LoadAligned(ourWeights + 121 * N);
            w2 = VectorT.LoadAligned(ourWeights + 122 * N);
            w3 = VectorT.LoadAligned(ourWeights + 123 * N);
            v0 = VectorT.LoadAligned(ourData + 120 * N);
            v1 = VectorT.LoadAligned(ourData + 121 * N);
            v2 = VectorT.LoadAligned(ourData + 122 * N);
            v3 = VectorT.LoadAligned(ourData + 123 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(ourWeights + 124 * N);
            w1 = VectorT.LoadAligned(ourWeights + 125 * N);
            w2 = VectorT.LoadAligned(ourWeights + 126 * N);
            w3 = VectorT.LoadAligned(ourWeights + 127 * N);
            v0 = VectorT.LoadAligned(ourData + 124 * N);
            v1 = VectorT.LoadAligned(ourData + 125 * N);
            v2 = VectorT.LoadAligned(ourData + 126 * N);
            v3 = VectorT.LoadAligned(ourData + 127 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 0 * N);
            w1 = VectorT.LoadAligned(theirWeights + 1 * N);
            w2 = VectorT.LoadAligned(theirWeights + 2 * N);
            w3 = VectorT.LoadAligned(theirWeights + 3 * N);
            v0 = VectorT.LoadAligned(theirData + 0 * N);
            v1 = VectorT.LoadAligned(theirData + 1 * N);
            v2 = VectorT.LoadAligned(theirData + 2 * N);
            v3 = VectorT.LoadAligned(theirData + 3 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 4 * N);
            w1 = VectorT.LoadAligned(theirWeights + 5 * N);
            w2 = VectorT.LoadAligned(theirWeights + 6 * N);
            w3 = VectorT.LoadAligned(theirWeights + 7 * N);
            v0 = VectorT.LoadAligned(theirData + 4 * N);
            v1 = VectorT.LoadAligned(theirData + 5 * N);
            v2 = VectorT.LoadAligned(theirData + 6 * N);
            v3 = VectorT.LoadAligned(theirData + 7 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 8 * N);
            w1 = VectorT.LoadAligned(theirWeights + 9 * N);
            w2 = VectorT.LoadAligned(theirWeights + 10 * N);
            w3 = VectorT.LoadAligned(theirWeights + 11 * N);
            v0 = VectorT.LoadAligned(theirData + 8 * N);
            v1 = VectorT.LoadAligned(theirData + 9 * N);
            v2 = VectorT.LoadAligned(theirData + 10 * N);
            v3 = VectorT.LoadAligned(theirData + 11 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 12 * N);
            w1 = VectorT.LoadAligned(theirWeights + 13 * N);
            w2 = VectorT.LoadAligned(theirWeights + 14 * N);
            w3 = VectorT.LoadAligned(theirWeights + 15 * N);
            v0 = VectorT.LoadAligned(theirData + 12 * N);
            v1 = VectorT.LoadAligned(theirData + 13 * N);
            v2 = VectorT.LoadAligned(theirData + 14 * N);
            v3 = VectorT.LoadAligned(theirData + 15 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 16 * N);
            w1 = VectorT.LoadAligned(theirWeights + 17 * N);
            w2 = VectorT.LoadAligned(theirWeights + 18 * N);
            w3 = VectorT.LoadAligned(theirWeights + 19 * N);
            v0 = VectorT.LoadAligned(theirData + 16 * N);
            v1 = VectorT.LoadAligned(theirData + 17 * N);
            v2 = VectorT.LoadAligned(theirData + 18 * N);
            v3 = VectorT.LoadAligned(theirData + 19 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 20 * N);
            w1 = VectorT.LoadAligned(theirWeights + 21 * N);
            w2 = VectorT.LoadAligned(theirWeights + 22 * N);
            w3 = VectorT.LoadAligned(theirWeights + 23 * N);
            v0 = VectorT.LoadAligned(theirData + 20 * N);
            v1 = VectorT.LoadAligned(theirData + 21 * N);
            v2 = VectorT.LoadAligned(theirData + 22 * N);
            v3 = VectorT.LoadAligned(theirData + 23 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 24 * N);
            w1 = VectorT.LoadAligned(theirWeights + 25 * N);
            w2 = VectorT.LoadAligned(theirWeights + 26 * N);
            w3 = VectorT.LoadAligned(theirWeights + 27 * N);
            v0 = VectorT.LoadAligned(theirData + 24 * N);
            v1 = VectorT.LoadAligned(theirData + 25 * N);
            v2 = VectorT.LoadAligned(theirData + 26 * N);
            v3 = VectorT.LoadAligned(theirData + 27 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 28 * N);
            w1 = VectorT.LoadAligned(theirWeights + 29 * N);
            w2 = VectorT.LoadAligned(theirWeights + 30 * N);
            w3 = VectorT.LoadAligned(theirWeights + 31 * N);
            v0 = VectorT.LoadAligned(theirData + 28 * N);
            v1 = VectorT.LoadAligned(theirData + 29 * N);
            v2 = VectorT.LoadAligned(theirData + 30 * N);
            v3 = VectorT.LoadAligned(theirData + 31 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 32 * N);
            w1 = VectorT.LoadAligned(theirWeights + 33 * N);
            w2 = VectorT.LoadAligned(theirWeights + 34 * N);
            w3 = VectorT.LoadAligned(theirWeights + 35 * N);
            v0 = VectorT.LoadAligned(theirData + 32 * N);
            v1 = VectorT.LoadAligned(theirData + 33 * N);
            v2 = VectorT.LoadAligned(theirData + 34 * N);
            v3 = VectorT.LoadAligned(theirData + 35 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 36 * N);
            w1 = VectorT.LoadAligned(theirWeights + 37 * N);
            w2 = VectorT.LoadAligned(theirWeights + 38 * N);
            w3 = VectorT.LoadAligned(theirWeights + 39 * N);
            v0 = VectorT.LoadAligned(theirData + 36 * N);
            v1 = VectorT.LoadAligned(theirData + 37 * N);
            v2 = VectorT.LoadAligned(theirData + 38 * N);
            v3 = VectorT.LoadAligned(theirData + 39 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 40 * N);
            w1 = VectorT.LoadAligned(theirWeights + 41 * N);
            w2 = VectorT.LoadAligned(theirWeights + 42 * N);
            w3 = VectorT.LoadAligned(theirWeights + 43 * N);
            v0 = VectorT.LoadAligned(theirData + 40 * N);
            v1 = VectorT.LoadAligned(theirData + 41 * N);
            v2 = VectorT.LoadAligned(theirData + 42 * N);
            v3 = VectorT.LoadAligned(theirData + 43 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 44 * N);
            w1 = VectorT.LoadAligned(theirWeights + 45 * N);
            w2 = VectorT.LoadAligned(theirWeights + 46 * N);
            w3 = VectorT.LoadAligned(theirWeights + 47 * N);
            v0 = VectorT.LoadAligned(theirData + 44 * N);
            v1 = VectorT.LoadAligned(theirData + 45 * N);
            v2 = VectorT.LoadAligned(theirData + 46 * N);
            v3 = VectorT.LoadAligned(theirData + 47 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 48 * N);
            w1 = VectorT.LoadAligned(theirWeights + 49 * N);
            w2 = VectorT.LoadAligned(theirWeights + 50 * N);
            w3 = VectorT.LoadAligned(theirWeights + 51 * N);
            v0 = VectorT.LoadAligned(theirData + 48 * N);
            v1 = VectorT.LoadAligned(theirData + 49 * N);
            v2 = VectorT.LoadAligned(theirData + 50 * N);
            v3 = VectorT.LoadAligned(theirData + 51 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 52 * N);
            w1 = VectorT.LoadAligned(theirWeights + 53 * N);
            w2 = VectorT.LoadAligned(theirWeights + 54 * N);
            w3 = VectorT.LoadAligned(theirWeights + 55 * N);
            v0 = VectorT.LoadAligned(theirData + 52 * N);
            v1 = VectorT.LoadAligned(theirData + 53 * N);
            v2 = VectorT.LoadAligned(theirData + 54 * N);
            v3 = VectorT.LoadAligned(theirData + 55 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 56 * N);
            w1 = VectorT.LoadAligned(theirWeights + 57 * N);
            w2 = VectorT.LoadAligned(theirWeights + 58 * N);
            w3 = VectorT.LoadAligned(theirWeights + 59 * N);
            v0 = VectorT.LoadAligned(theirData + 56 * N);
            v1 = VectorT.LoadAligned(theirData + 57 * N);
            v2 = VectorT.LoadAligned(theirData + 58 * N);
            v3 = VectorT.LoadAligned(theirData + 59 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 60 * N);
            w1 = VectorT.LoadAligned(theirWeights + 61 * N);
            w2 = VectorT.LoadAligned(theirWeights + 62 * N);
            w3 = VectorT.LoadAligned(theirWeights + 63 * N);
            v0 = VectorT.LoadAligned(theirData + 60 * N);
            v1 = VectorT.LoadAligned(theirData + 61 * N);
            v2 = VectorT.LoadAligned(theirData + 62 * N);
            v3 = VectorT.LoadAligned(theirData + 63 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 64 * N);
            w1 = VectorT.LoadAligned(theirWeights + 65 * N);
            w2 = VectorT.LoadAligned(theirWeights + 66 * N);
            w3 = VectorT.LoadAligned(theirWeights + 67 * N);
            v0 = VectorT.LoadAligned(theirData + 64 * N);
            v1 = VectorT.LoadAligned(theirData + 65 * N);
            v2 = VectorT.LoadAligned(theirData + 66 * N);
            v3 = VectorT.LoadAligned(theirData + 67 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 68 * N);
            w1 = VectorT.LoadAligned(theirWeights + 69 * N);
            w2 = VectorT.LoadAligned(theirWeights + 70 * N);
            w3 = VectorT.LoadAligned(theirWeights + 71 * N);
            v0 = VectorT.LoadAligned(theirData + 68 * N);
            v1 = VectorT.LoadAligned(theirData + 69 * N);
            v2 = VectorT.LoadAligned(theirData + 70 * N);
            v3 = VectorT.LoadAligned(theirData + 71 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 72 * N);
            w1 = VectorT.LoadAligned(theirWeights + 73 * N);
            w2 = VectorT.LoadAligned(theirWeights + 74 * N);
            w3 = VectorT.LoadAligned(theirWeights + 75 * N);
            v0 = VectorT.LoadAligned(theirData + 72 * N);
            v1 = VectorT.LoadAligned(theirData + 73 * N);
            v2 = VectorT.LoadAligned(theirData + 74 * N);
            v3 = VectorT.LoadAligned(theirData + 75 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 76 * N);
            w1 = VectorT.LoadAligned(theirWeights + 77 * N);
            w2 = VectorT.LoadAligned(theirWeights + 78 * N);
            w3 = VectorT.LoadAligned(theirWeights + 79 * N);
            v0 = VectorT.LoadAligned(theirData + 76 * N);
            v1 = VectorT.LoadAligned(theirData + 77 * N);
            v2 = VectorT.LoadAligned(theirData + 78 * N);
            v3 = VectorT.LoadAligned(theirData + 79 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 80 * N);
            w1 = VectorT.LoadAligned(theirWeights + 81 * N);
            w2 = VectorT.LoadAligned(theirWeights + 82 * N);
            w3 = VectorT.LoadAligned(theirWeights + 83 * N);
            v0 = VectorT.LoadAligned(theirData + 80 * N);
            v1 = VectorT.LoadAligned(theirData + 81 * N);
            v2 = VectorT.LoadAligned(theirData + 82 * N);
            v3 = VectorT.LoadAligned(theirData + 83 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 84 * N);
            w1 = VectorT.LoadAligned(theirWeights + 85 * N);
            w2 = VectorT.LoadAligned(theirWeights + 86 * N);
            w3 = VectorT.LoadAligned(theirWeights + 87 * N);
            v0 = VectorT.LoadAligned(theirData + 84 * N);
            v1 = VectorT.LoadAligned(theirData + 85 * N);
            v2 = VectorT.LoadAligned(theirData + 86 * N);
            v3 = VectorT.LoadAligned(theirData + 87 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 88 * N);
            w1 = VectorT.LoadAligned(theirWeights + 89 * N);
            w2 = VectorT.LoadAligned(theirWeights + 90 * N);
            w3 = VectorT.LoadAligned(theirWeights + 91 * N);
            v0 = VectorT.LoadAligned(theirData + 88 * N);
            v1 = VectorT.LoadAligned(theirData + 89 * N);
            v2 = VectorT.LoadAligned(theirData + 90 * N);
            v3 = VectorT.LoadAligned(theirData + 91 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 92 * N);
            w1 = VectorT.LoadAligned(theirWeights + 93 * N);
            w2 = VectorT.LoadAligned(theirWeights + 94 * N);
            w3 = VectorT.LoadAligned(theirWeights + 95 * N);
            v0 = VectorT.LoadAligned(theirData + 92 * N);
            v1 = VectorT.LoadAligned(theirData + 93 * N);
            v2 = VectorT.LoadAligned(theirData + 94 * N);
            v3 = VectorT.LoadAligned(theirData + 95 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 96 * N);
            w1 = VectorT.LoadAligned(theirWeights + 97 * N);
            w2 = VectorT.LoadAligned(theirWeights + 98 * N);
            w3 = VectorT.LoadAligned(theirWeights + 99 * N);
            v0 = VectorT.LoadAligned(theirData + 96 * N);
            v1 = VectorT.LoadAligned(theirData + 97 * N);
            v2 = VectorT.LoadAligned(theirData + 98 * N);
            v3 = VectorT.LoadAligned(theirData + 99 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 100 * N);
            w1 = VectorT.LoadAligned(theirWeights + 101 * N);
            w2 = VectorT.LoadAligned(theirWeights + 102 * N);
            w3 = VectorT.LoadAligned(theirWeights + 103 * N);
            v0 = VectorT.LoadAligned(theirData + 100 * N);
            v1 = VectorT.LoadAligned(theirData + 101 * N);
            v2 = VectorT.LoadAligned(theirData + 102 * N);
            v3 = VectorT.LoadAligned(theirData + 103 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 104 * N);
            w1 = VectorT.LoadAligned(theirWeights + 105 * N);
            w2 = VectorT.LoadAligned(theirWeights + 106 * N);
            w3 = VectorT.LoadAligned(theirWeights + 107 * N);
            v0 = VectorT.LoadAligned(theirData + 104 * N);
            v1 = VectorT.LoadAligned(theirData + 105 * N);
            v2 = VectorT.LoadAligned(theirData + 106 * N);
            v3 = VectorT.LoadAligned(theirData + 107 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 108 * N);
            w1 = VectorT.LoadAligned(theirWeights + 109 * N);
            w2 = VectorT.LoadAligned(theirWeights + 110 * N);
            w3 = VectorT.LoadAligned(theirWeights + 111 * N);
            v0 = VectorT.LoadAligned(theirData + 108 * N);
            v1 = VectorT.LoadAligned(theirData + 109 * N);
            v2 = VectorT.LoadAligned(theirData + 110 * N);
            v3 = VectorT.LoadAligned(theirData + 111 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 112 * N);
            w1 = VectorT.LoadAligned(theirWeights + 113 * N);
            w2 = VectorT.LoadAligned(theirWeights + 114 * N);
            w3 = VectorT.LoadAligned(theirWeights + 115 * N);
            v0 = VectorT.LoadAligned(theirData + 112 * N);
            v1 = VectorT.LoadAligned(theirData + 113 * N);
            v2 = VectorT.LoadAligned(theirData + 114 * N);
            v3 = VectorT.LoadAligned(theirData + 115 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 116 * N);
            w1 = VectorT.LoadAligned(theirWeights + 117 * N);
            w2 = VectorT.LoadAligned(theirWeights + 118 * N);
            w3 = VectorT.LoadAligned(theirWeights + 119 * N);
            v0 = VectorT.LoadAligned(theirData + 116 * N);
            v1 = VectorT.LoadAligned(theirData + 117 * N);
            v2 = VectorT.LoadAligned(theirData + 118 * N);
            v3 = VectorT.LoadAligned(theirData + 119 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 120 * N);
            w1 = VectorT.LoadAligned(theirWeights + 121 * N);
            w2 = VectorT.LoadAligned(theirWeights + 122 * N);
            w3 = VectorT.LoadAligned(theirWeights + 123 * N);
            v0 = VectorT.LoadAligned(theirData + 120 * N);
            v1 = VectorT.LoadAligned(theirData + 121 * N);
            v2 = VectorT.LoadAligned(theirData + 122 * N);
            v3 = VectorT.LoadAligned(theirData + 123 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
            ...
            w0 = VectorT.LoadAligned(theirWeights + 124 * N);
            w1 = VectorT.LoadAligned(theirWeights + 125 * N);
            w2 = VectorT.LoadAligned(theirWeights + 126 * N);
            w3 = VectorT.LoadAligned(theirWeights + 127 * N);
            v0 = VectorT.LoadAligned(theirData + 124 * N);
            v1 = VectorT.LoadAligned(theirData + 125 * N);
            v2 = VectorT.LoadAligned(theirData + 126 * N);
            v3 = VectorT.LoadAligned(theirData + 127 * N);
            clip0 = VectorT.Min(VectorT.Max(v0, zeroVec), maxVec);
            clip1 = VectorT.Min(VectorT.Max(v1, zeroVec), maxVec);
            clip2 = VectorT.Min(VectorT.Max(v2, zeroVec), maxVec);
            clip3 = VectorT.Min(VectorT.Max(v3, zeroVec), maxVec);
