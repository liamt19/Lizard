...
namespace Lizard.Logic.NN
{
    ...
    public static unsafe partial class Bucketed768
    {
        ...
        public const string NetworkName = "morelayers_1280x5_16_32_1-550-params.bin";
        ...
        public static void Initialize(string networkToLoad, bool exitIfFail = true)
        {
            ...
            NetStats("L1 weight", Net.L1Weights, N_L1W / OUTPUT_BUCKETS);
            NetStats("L1 bias\t", Net.L1Biases, N_L1B / OUTPUT_BUCKETS);

            NetStats("L2 weight", Net.L2Weights, N_L2W / OUTPUT_BUCKETS);
            NetStats("L2 bias\t", Net.L2Biases, N_L2B / OUTPUT_BUCKETS);

            NetStats("L3 weight", Net.L3Weights, N_L3W / OUTPUT_BUCKETS);
...
#if USE_AVX2
            ActivateFTAndAffineL1Sparse(ourData,   Net.L1Weights, L1OutputsUs);
            ActivateFTAndAffineL1Sparse(theirData, Net.L1Weights + L1_SIZE * L2_SIZE, L1OutputsThem);
#else
            ActivateFTAndAffineL1Fallback(ourData,   Net.L1Weights, L1OutputsUs);
            ActivateFTAndAffineL1Fallback(theirData, Net.L1Weights + L1_SIZE * L2_SIZE, L1OutputsThem);
#endif

            for (int i = 0; i < L2_SIZE; ++i)
            {
                L2Inputs[i] = ((float)L1OutputsUs[i] + L1OutputsThem[i]) / ((float)FT_QUANT * L1_QUANT) + Net.L1Biases[i];
            }

#if USE_AVX2
            ActivateL1AndAffineL2(L2Inputs,  Net.L2Weights, Net.L2Biases, L2Outputs);
            ActivateL2AndAffineL3(L2Outputs, Net.L3Weights, Net.L3Biases[0], ref L3Output);
#else
            ActivateL1AndAffineL2Fallback(L2Inputs,  Net.L2Weights, Net.L2Biases, L2Outputs);
            ActivateL2AndAffineL3Fallback(L2Outputs, Net.L3Weights, Net.L3Biases, ref L3Output);
#endif
